...
def calculate_reads_counts(bam_file, elements_info_path, output_path):
    ...
    for _, row in elements_info.iterrows():
        hgt_id = row['HGT_ID']
        contig_name = row['contig_name']  # Use contig_name column
        contig_start = row['contig_start']  # Changed back to 'contig_start'
        contig_end = row['contig_end']      # Changed back to 'contig_end'
