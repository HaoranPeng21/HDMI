#!/bin/bash
#SBATCH --job-name=QC_BBDuK_KneadData_retry
#SBATCH --output=log/QC_BBDuK_KneadData_retry_%A_%a.out
#SBATCH --error=log/QC_BBDuK_KneadData_retry_%A_%a.err
...
#SBATCH --array=1-286%50

# Sewage samples QC and trimming pipeline - RETRY VERSION
# Based on HIV virus pipeline, adapted for sewage metagenomics
# Increased time and memory limits for failed samples

sample_dir=/scratch/p312334/project/13--Holo/sewage_sequences
sample_list=/scratch/p312334/project/13--Holo/script/failed_samples_complete.txt
SAMPLE_ID=$(sed "${SLURM_ARRAY_TASK_ID}q;d" ${sample_list})

echo -e '\n-------------------- WORKING WITH '${SAMPLE_ID}' SAMPLE (RETRY) --------------------'
...
# Run FASTQC on downloaded sewage sequences
fastqc -o QC_RESULTS/INITIAL/${SAMPLE_ID} -t ${SLURM_CPUS_PER_TASK} ${sample_dir}/${SAMPLE_ID}_1.gz 
fastqc -o QC_RESULTS/INITIAL/${SAMPLE_ID} -t ${SLURM_CPUS_PER_TASK} ${sample_dir}/${SAMPLE_ID}_2.gz

# Calculate number of raw reads and bases
N_reads_1=$( zcat ${sample_dir}/${SAMPLE_ID}_1.gz | echo $((`wc -l`/4)) ) 
N_bases_1=$( zcat ${sample_dir}/${SAMPLE_ID}_1.gz | paste - - - - | cut -f2 | wc -c )
N_reads_2=$( zcat ${sample_dir}/${SAMPLE_ID}_2.gz | echo $((`wc -l`/4)) )
N_bases_2=$( zcat ${sample_dir}/${SAMPLE_ID}_2.gz | paste - - - - | cut -f2 | wc -c )
...
cp ${sample_dir}/${SAMPLE_ID}_1.gz ${TMPDIR}/${SAMPLE_ID}/filtering_data/
cp ${sample_dir}/${SAMPLE_ID}_2.gz ${TMPDIR}/${SAMPLE_ID}/filtering_data/

echo -e '\n---- RUNNING BBDuK for adapter trimming ----'
...
bbduk.sh \
    in1=${TMPDIR}/${SAMPLE_ID}/filtering_data/${SAMPLE_ID}_1.gz \
    in2=${TMPDIR}/${SAMPLE_ID}/filtering_data/${SAMPLE_ID}_2.gz \
...
echo -e '\n---- RUNNING KneadData for human contamination removal and quality trimming ----'
...
N_bases_1=$( cat KneadData_RESULTS/${SAMPLE_ID}/${SAMPLE_ID}_kneaddata_paired_1.fastq | paste - - - - | cut -f2 | wc -c )

N_reads_2=$( cat KneadData_RESULTS/${SAMPLE_ID}/${SAMPLE_ID}_kneaddata_paired_2.fastq | echo $((`wc -l`/4)) )
...
mkdir -p metaSPAdes/CLEAN_READS
mkdir -p metaSPAdes/UNMATCHED_READS

rsync -av $(find KneadData_RESULTS/${SAMPLE_ID} -name "*paired*.fastq.gz" -type f) metaSPAdes/CLEAN_READS/
rsync -av $(find KneadData_RESULTS/${SAMPLE_ID} -name "*unmatched*fastq.gz" -type f) metaSPAdes/UNMATCHED_READS/

ls metaSPAdes/CLEAN_READS/ | awk -F'_kneaddata' '{print $1}' | uniq > metaSPAdes/CLEAN_READS/list.txt
...
if [ $(find metaSPAdes/CLEAN_READS/ -type f -name "*${SAMPLE_ID}_kneaddata_paired*.fastq.gz" | wc -l) -eq 2 ]; then
...
if [ $(find metaSPAdes/UNMATCHED_READS/ -type f -name "*${SAMPLE_ID}_kneaddata_unmatched*.fastq.gz" | wc -l) -eq 1 ]; then
...
echo -e '\n---- Cleaning up raw data to save storage space ----'

# Check if clean data was successfully generated and remove raw data
if [ -f "metaSPAdes/CLEAN_READS/${SAMPLE_ID}_kneaddata_paired_1.fastq.gz" ] && \
   [ -f "metaSPAdes/CLEAN_READS/${SAMPLE_ID}_kneaddata_paired_2.fastq.gz" ]; then
    
    echo "Clean data successfully generated for ${SAMPLE_ID}, removing raw data..."
    
    # Remove raw FASTQ files to save storage
    if [ -f "${sample_dir}/${SAMPLE_ID}_1.gz" ]; then
        rm "${sample_dir}/${SAMPLE_ID}_1.gz"
        echo "Removed ${sample_dir}/${SAMPLE_ID}_1.gz"
    fi
    
    if [ -f "${sample_dir}/${SAMPLE_ID}_2.gz" ]; then
        rm "${sample_dir}/${SAMPLE_ID}_2.gz"
        echo "Removed ${sample_dir}/${SAMPLE_ID}_2.gz"
    fi
    
    echo "Raw data cleanup completed for ${SAMPLE_ID}"
else
    echo "Warning: Clean data not found for ${SAMPLE_ID}, keeping raw data for debugging"
fi

echo -e '\n---- Read QC, adapter/contaminant trimming and compressing DONE for sewage sample (RETRY) ----'
